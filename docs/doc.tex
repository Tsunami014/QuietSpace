...
\begin{figure}[H]
  ...
  \begin{structc}
    ...
    \node[module] (choosewrld) at (-1.5, -2) {Choose world};
        \draw[to] (startup) --
            node[pos=0.4,data] {\dfdData{Dat}{1}[-90][east]}
          (choosewrld);

    \node[module] (load) at (0.5, -2.25) {Load world};
        \draw[to] (gameplay) --
          (load);
    \node[module] (play) at (2, -2.25) {Play in world};
        \draw[to] (gameplay) --
            node[pos=0.0,diamondmark] {}
            node[pos=0.35,data] {\dfdData[black]{Dat2}{-1}}
            node[pos=0.8,data] {\dfdData{Dat}{1}}
          (play);
