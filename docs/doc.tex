...
\begin{figure}[H]
  ...
  \begin{dfd}
    ...
    \matrix{
      ...
      \&
    \\
         \node[process,sized] (displ) {Render\\display};
    ...
    \draw[to] (worlddat) --
      ...
      (displ);
    \draw[to] (interac) to[bend left=5]
    ...
    \draw[to] (playr) to[bend right=25]
        node[midway,left,align=right] {Refresh\\screen}
      (displ);
  \end{dfd}
  \end{center}
\end{figure}
...
\begin{figure}[H]
  ...
  \begin{structc}
    ...
    \node[module] (startup) at (-1, -1) {Startup};
        \draw[to] (main.south) --
          (startup.north);
    ...
    \draw[to,draw=none] (main.south) --
        node[pos=1,repeat] {\dfdLoop[0.4]{Until user quits}}
      ($(main.south) + (0,-0.35)$);

    \node[module] (load) at (1, -2) {Load world};
        \draw[to] (gameplay.south) -- (load);
    \node[module] (play) at (2.5, -2) {Play world};
        \draw[to] (gameplay.south) -- (play);
    \node[module] (save) at (4, -2) {Save world};
        \draw[to] (gameplay.south) -- (save);

    \node[module] (xprt) at (-0.5, -2) {Export world};
        \draw[to] (startup.south) --
            node[pos=0.6,data={World\\ID}] {\dfdData}
          (xprt.north);
    \node[module] (choose) at (-2, -2) {Choose world\\to play};
        \draw[to] (startup.south) --
            node[pos=0.6,data={World\\ID}] {\dfdData[black]}
         (choose.north);
    \draw[to,draw=none] (startup.south) --
        node[pos=0,diamondflip] {}
      ($(startup.south) + (-0.2,-1)$);
